...
Public Class calProgramacaoCalibracao
...
#Region "CARREGA LISTA"
    ...
    Public Sub carregaListaDatas()
        ...
        Dim sqlId As String = String.Format("SELECT * FROM CALIBRACOES_PLANO")
        dt = Manager.Util.getDataTableById(sqlId)

        'Monta o Grid
        gridPlanoDatas.AutoGenerateColumns = False
        gridPlanoDatas.DataSource = Manager.Util.getDataTable("SELECT * FROM VW_LISTAGEM_CALIBRACOES_INSTRUMENTOS_PLANO_ANUAL")
...
#Region "CARREGA LISTA - FILTRO"
    ...
    Public Sub carregaListaDatasAno(ByVal ANO As String)
        ...
        If ANO = "" Then
            ANO = 0
        End If
        Dim sqlId2 As String = String.Format("SELECT * FROM VW_LISTAGEM_CALIBRACOES_INSTRUMENTOS_PLANO_ANUAL WHERE ANO = {0}", ANO)
...
#Region "POPULA COMBOS"
    ...
    Sub CarregaComboAno()
        With comboAno
            .DataSource = Manager.Util.getDataTable("SELECT DISTINCT ANO FROM VW_LISTAGEM_CALIBRACOES_INSTRUMENTOS_PLANO_ANUAL")
            .DisplayMember = "ANO"
            .ValueMember = "ANO"
        End With
    End Sub
...
#Region "LOAD"
    ...
    Private Sub CalProgramacaoAnual_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region "EXCLUIR"
    ...
    Private Sub botExcluir_Click(sender As System.Object, e As System.EventArgs) Handles botExcluir.Click
        ...
        If gridPlanoDatas.RowSel <= 0 Then
            MessageBox.Show("Não à nenhuma Programação cadastrada", "Programação de Calibrações", Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        Else
            ...
            Try
                'Exluindo Plano
                If MessageBox.Show("Tem certeza que deseja excluir o item selecionado ?", "Programação de Calibrações", Windows.Forms.MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation) = Windows.Forms.DialogResult.OK Then
                    plano.DeletePlano(gridPlanoDatas(gridPlanoDatas.RowSel, 1))
                    MessageBox.Show("Plano excluído com sucesso", "Programação de Calibrações", Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Information)
                    carregaListaDatas()
                End If
...
#Region "REALIZACAO"
    ...
    Public Sub Realizacao(ByVal t As Integer, ByVal id As Integer, ByVal mes As String)
        ...
        Dim sql As String = String.Format("SELECT * FROM CALIBRACOES_PLANO WHERE ID_CALIBRACOES_PLANO = {0}", id)
        ...
        Try
            ...
            While dr.Read
                ...
                'PROGRAMADA
                If t = 1 Then
                    inst = dr("ID_CALIBRACOES_INSTRUMENTOS")
                    tp = dr("TIPO")
                    Ano = dr("ANO")
                    Sit = "Programada"
                    Dim sql2 As String = String.Format("SELECT * FROM CALIBRACOES_REALIZAR WHERE ID_CALIBRACOES_PLANO = {0}", id)
                    Dim dr2 As SqlDataReader = Manager.Util.getDataReader(sql2)

                    If dr2.HasRows = True Then
                        While dr2.Read
                            If id = dr2("ID_CALIBRACOES_PLANO") And inst = dr2("ID_CALIBRACOES_INSTRUMENTOS") And tp = dr2("TIPO") And Ano = dr2("ANO") And mes = dr2("mes") Then
                                Check.DeleteRealiza(id, inst, Ano, mes)
                                Check.AddRealiza(inst, (id), tp, Ano, mes, Sit)
                                Exit Sub
                            End If
                        End While
                        Check.AddRealiza(inst, (id), tp, Ano, mes, Sit)
