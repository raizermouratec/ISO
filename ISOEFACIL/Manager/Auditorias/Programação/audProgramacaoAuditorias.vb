...
Imports System.IO
Imports ISOEFACIL.audRealizarAuditorias
Public Class audProgramacaoAuditorias
...
#Region "CARREGA LISTA"
    ...
    Public Sub carregaListaDatas()
        ...
        Dim sqlId As String = String.Format("SELECT * FROM AUDITORIAS_PLANO")
        dt = Manager.Util.getDataTableById(sqlId)

        'Monta o Grid
        gridPlanoDatas.AutoGenerateColumns = False
        gridPlanoDatas.DataSource = Manager.Util.getDataTable("SELECT * FROM VW_LISTAGEM_AUDITORIAS_PLANO_ANUAL")
...
#Region "CARREGA LISTA - FILTRO"
    ...
    Public Sub carregaListaDatasAno(ByVal ANO As String)
        ...
        If ANO = "" Then
            ANO = 0
        End If
        Dim sqlId2 As String = String.Format("SELECT * FROM VW_LISTAGEM_AUDITORIAS_PLANO_ANUAL WHERE ANO = {0}", ANO)
...
#Region "LOAD"
    ...
    Private Sub audProgramacaoAuditorias_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
...
#Region "EXCLUIR"
    ...
    Private Sub botExcluir_Click(sender As System.Object, e As System.EventArgs) Handles botExcluir.Click
        ...
        If gridPlanoDatas.RowSel <= 0 Then
            MessageBox.Show("Não à nenhuma Programação cadastrada", "Programação de Auditorias", Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        Else
            Dim auditores As New planoAnualAuditoria

            Try
                'Exluindo Plano
                If MessageBox.Show("Tem certeza que deseja excluir o item selecionado ?", "Programação de Auditorias", Windows.Forms.MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation) = Windows.Forms.DialogResult.OK Then
                    auditores.DeletePlano(gridPlanoDatas(gridPlanoDatas.RowSel, 1))
                    MessageBox.Show("Plano excluído com sucesso", "Programação de Auditorias", Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Information)
                    carregaListaDatas()
                End If
...
#Region "ITENS"
    ...
    Private Sub botItens_Click(sender As System.Object, e As System.EventArgs) Handles botItens.Click

        Dim Check As New planoAnualAuditoria
        Dim audItem As New audItensProgAuditorias
        Dim id As Integer
        Dim ano As String
            ...
            With gridPlanoDatas
                ...
                If .ColSel = 2 Then
                    Try
                        ...
                        audItem.idItem = id
                        audItem.EditarItensAuditoria()
                    Catch ex As Exception
                        MsgBox("Não foi possivel realizar a operação. Tente novamente.")
                    End Try
                Else
                    MsgBox("Por favor Selecione uma auditoria.", MsgBoxStyle.Information, "Iso é Fácil")
                    Exit Sub
                End If
...
#Region "CANCELAR"
    ...
    Private Sub cmdCancelar_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles cmdCancelar.Click
        ...
        Dim ms As DialogResult = MessageBox.Show("Deseja realmente cancelar a Auditoria ?", "Plano Anual de Auditorias", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
...
#Region "REPROGRAMAR"
    ...
    Private Sub cmdReprogramar_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles cmdReprogramar.Click
        ...
        Dim ms As DialogResult = MessageBox.Show("Deseja realmente Reprogramar a Auditoria ?", "Plano Anual de Auditorias", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
...
#Region "REMOVER"
    ...
    Private Sub cmdRemover_Click(ByVal sender As System.Object, ByVal e As C1.Win.C1Command.ClickEventArgs) Handles cmdRemover.Click
        ...
        Dim ms As DialogResult = MessageBox.Show("Deseja realmente Remover a Auditoria ?", "Plano Anual de Auditoria", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
        ...
        Try
            ...
            While dr.Read
                ...
                If t = 1 Then
                    ...
                    Ano = dr("ANO")
                    Sit = "Programada"
                    Dim sql2 As String = String.Format("SELECT * FROM AUDITORIAS_REALIZAR WHERE ID_AUDITORIAS_PLANO = {0}", id)
                    Dim dr2 As SqlDataReader = Manager.Util.getDataReader(sql2)

                    If dr2.HasRows = True Then
                        While dr2.Read
                            If id = dr2("ID_AUDITORIAS_PLANO") And area = dr2("ID_AREAS") And Ano = dr2("ANO") And mes = dr2("mes") Then
                                Check.DeleteRealiza(id, area, Ano, mes)
                                Check.AddRealizaAuditoria(area, (id), Ano, mes, Sit)
                                Exit Sub
                            End If
                        End While
                        Check.AddRealizaAuditoria(area, (id), Ano, mes, Sit)
